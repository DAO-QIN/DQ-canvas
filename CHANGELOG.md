...
- [Canvas/任务] 生成任务改为每次主动提交使用新的 submission ID，持久化 source/target 节点身份并在撤销、删除、替换时失效旧运行令牌。任务面板按活跃状态直接查询，显示真实阶段、百分比、耗时及真实计费或“未计费”。
- [Canvas/连线] 悬停或选中连线时，会在逻辑中点显示可拖动的 `+` 插入柄；拖到空白画布后选择组件会将原连线拆分为“来源 → 新节点 → 目标”，保留外侧锚点比例，拖到兼容节点则沿用现有端点吸附与方向规则。该交互支持鼠标、触控、键盘、取消事件和无效配置过滤。
- [Canvas/图片工具] 图片节点保留原有工具并新增标注、5x5 情绪与 FaceCap 3D 表情预览、人物质感、多视角和位置尺寸锁定；锁定只约束节点位置与尺寸，内容、提示词和参数仍可编辑。快捷 Dock 默认固定 7 项、最多 8 项，锁定入口独立显示并使用放大加粗徽标，更多菜单可配置功能名显示且同类图标统一对齐。新能力沿用 DQ 画布主题，并补齐派生节点、参考图比例、蒙版局部合成、刷新恢复、失败重试及源图身份保护。
- [Canvas/抠图] 图片组件新增基于 rembg CPU sidecar 的“抠图（移除背景）”工具：仅接受 JPEG/PNG/WebP，输入与输出各不超过 30MB；EXIF 校正后最长边超过 2048px 或源图超过 6400 万像素时自动等比缩至 2K，小图不放大，结果强制为透明 RGBA PNG。默认模型改为 `silueta`，V3 自定义表仍可从五个预取模型逐任务选择并调整 Alpha 与蒙版后处理参数，V1/V2 历史快照继续按 `u2net` 读取。全站 rembg 推理并发固定为 1，PostgreSQL advisory lock、租约和 FIFO 领取保证多 Worker 不插队或超额执行，文件回退模式使用跨进程锁；每用户每分钟 30 次防滥用限制保留。运行中的抠图按钮会切换为红色终止图标，排队和推理任务都可完全终止，迟到结果不会落盘。四套内置拓扑统一提供 2 CPU、5 GiB、2 个 intra-op 计算线程和 1 个 inter-op 线程。
- [仓库/文档] GitHub 入口、更新检查、安装模板、Compose 与 GHCR 工作流统一迁到 `DAO-QIN/DQ-canvas`；补齐代码质量分类、Canvas 高级工具/快捷键、抠图链路与最新项目结构说明。
- [工程] Web、文档站、Docker 构建与 GitHub Quality 工作流统一固定 pnpm 10.34.5，并将补丁依赖锁定信息写为 pnpm 10 可冻结安装的结构，避免不同 pnpm 版本重写锁文件后导致 CI 安装失败。
- [Agent] `/create`、Canvas、图片/视频工作台与短剧 Agent 支持独立切换文本逻辑模型；未选择时跟随后台默认，显式选择会校验文本能力与可用渠道，并贯穿规划、重试、计费和退款，手动媒体模型模式保持原行为。
- [修复] Clash Verge / Mihomo TUN Fake-IP DNS 可通过显式环境开关兼容模型目录与能力检测，同时继续拒绝直接 Fake-IP、localhost、内网地址和云元数据域名。
...
- [修复] 友情链接后台尊重显式删除结果，默认仅保留 DQ-绘图；旧版内置的社区与 Linux.do 链接在归一化时清理，首页回退配置同步更新。
...
- [安装] 0.0.3 不支持沿用 0.0.2 数据库或原地升级。部署前必须删除旧数据库并重新安装项目，再通过 `/install` 完成初始化；安装页会分别生成加密密钥、外部维护令牌和独立 Worker 令牌，复制的 Compose 只把 Worker 令牌交给生成 Worker。安装完成后安装页自动关闭。
- [品牌/文档] 站点 Logo、ICO、favicon、manifest、登录页、管理后台和助手头像统一使用 DQ-绘图 品牌资源；README、部署说明、API 文档和数据库文档与当前发布保持一致。
...
- [文档] README 首屏改为公开首页截图；补充完整用户端、管理后台、平台能力与部署功能清单，以及仓库目录、关键文件职责、参与贡献指南、DQ 开源 QQ 交流群、赞助二维码和项目致谢；公开文案统一使用“画布”和“DQ-绘图”，并移除旧 QQ、爱发电及未确认的 Telegram/X/Instagram 默认入口。
