...
test("fresh deployments enter the installation flow", async ({ page }) => {
    ...
    await expect(page.getByText("DQ-绘图 安装向导", { exact: true })).toBeVisible();
...
test("public session omits internal configuration fields", async ({ request }) => {
    ...
    expect(serialized).not.toContain("DQ_");
});

test("installation rejects a wrong token and creates the first administrator once", async ({ page, request }) => {
    ...
    if (install.database?.schemaReady === false) {
        const rejected = await request.post("/api/install/initialize", { data: { installToken: "wrong-install-token" } });
