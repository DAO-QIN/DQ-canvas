...
import { FileImage, Globe2, Music2, Pencil, Settings2, Type, Video } from "lucide-react";
...
import { isCanvasComposerNode } from "../components/canvas-generation-composer-model";
...
import { CanvasCreativeWorkspaceChrome, CanvasSelectionSizeOverlay, type CanvasWorkspaceCreationAction } from "../components/creative-workspace-adapter";
import { canvasHandoffSelectionIds } from "./canvas-handoff-selection";
...
import { WorkspaceHandoffDialog, type WorkspaceHandoffTargetOption } from "@/components/creative-workspace";
import { isCreativeWorkspaceEnabled } from "@/lib/creative-workspace/feature-flags";
import { listDesignProjects } from "@/services/api/design-projects";
...
function DQCanvasPage() {
    ...
    const creativeWorkspaceEnabled = isCreativeWorkspaceEnabled("canvas");
    const handoffSelectionIds = canvasHandoffSelectionIds(nodes, [...selectedNodeIds]);
    const selectedWorkspaceComposerNode = selectedNodeIds.size === 1 ? nodes.find((node) => selectedNodeIds.has(node.id) && isCanvasComposerNode(node)) || null : null;
    const workspaceComposerNode = creativeWorkspaceEnabled ? selectedWorkspaceComposerNode : null;
    const creationActions: CanvasWorkspaceCreationAction[] = [
        { id: "text", label: "新建文本", icon: <Type />, onCreate: () => createNode(CanvasNodeType.Text) },
        { id: "image", label: "新建图片", icon: <FileImage />, onCreate: () => createNode(CanvasNodeType.Image) },
        { id: "panorama", label: "新建全景图", icon: <Globe2 />, onCreate: () => createNode(CanvasNodeType.Panorama) },
        { id: "drawing", label: "新建绘图", icon: <Pencil />, onCreate: () => createNode(CanvasNodeType.Drawing) },
        { id: "video", label: "新建视频", icon: <Video />, onCreate: () => createNode(CanvasNodeType.Video) },
        { id: "audio", label: "新建音频", icon: <Music2 />, onCreate: () => createNode(CanvasNodeType.Audio) },
        { id: "config", label: "新建生成配置", icon: <Settings2 />, onCreate: () => createNode(CanvasNodeType.Config) },
    ];
    const toggleAgent = () => (assistantOpen ? closeAgent() : openAgent());
    const assistantPanel = assistantMounted ? (
        <CanvasAssistantPanel
            ...
            onPrepareRun={prepareAgentRun}
            onExecuteWorkspaceActions={executeWorkspaceActions}
            onLocateNode={locateCanvasNode}
            onPasteImage={pasteAssistantImage}
            closing={assistantClosing}
            onCollapse={closeAgent}
            hosted={creativeWorkspaceEnabled}
        />
    ) : null;
    const handoffControl = creativeWorkspaceEnabled ? (
        <WorkspaceHandoffDialog
            ...
            sourceRevision={Number(currentProject?.revision) || 0}
            selectionIds={handoffSelectionIds}
            selectionLabel="已选图片素材"
            disabled={!projectLoaded || handoffSelectionIds.length === 0}
            loadTargets={async () => {
                const result = await listDesignProjects({ page: 1, pageSize: 100, status: "active" });
                return result.items.map<WorkspaceHandoffTargetOption>((item) => ({ id: item.id, title: item.title, revision: item.revision, surface: "design" }));
            }}
            flushSource={flushCurrentProject}
            getCurrentRevision={() => Number(useCanvasStore.getState().projects.find((item) => item.id === projectId)?.revision) || 0}
            onOpenTarget={(target) => router.push(`/design/${target.id}`)}
        />
    ) : null;
    const canvasContent = (
        <section className="relative h-full min-w-0 flex-1 overflow-hidden">
            {!creativeWorkspaceEnabled ? (
                <CanvasTopBar
                    ...
                    onWorkbench={() => void navigateToWorkbench()}
                    onProjects={() => void navigateToProjects()}
            ...
            <CanvasActiveTaskPanel projectId={projectId} tasks={activeGenerationTasks} />
            ...
            >
                ...
                {visibleNodes.map((node) => (
                    <CanvasNode
                        ...
                        showPanel={dialogNodeId === node.id && !selectionBox && (!creativeWorkspaceEnabled || node.type === CanvasNodeType.Config)}
            ...
            {!creativeWorkspaceEnabled ? <CanvasZoomControls scale={viewport.k} onScaleChange={setZoomScale} onReset={resetViewport} isMiniMapOpen={isMiniMapOpen} onToggleMiniMap={() => setIsMiniMapOpen((value) => !value)} /> : null}
            ...
            {cropNode?.metadata?.content ? <CanvasNodeCropDialog dataUrl={cropNode.metadata.content} open={Boolean(cropNode)} onClose={() => setCropNodeId(null)} onConfirm={(crop) => void cropImageNode(cropNode!, crop).catch(() => undefined)} /> : null}
            ...
            {emotionNode?.metadata?.content ? <CanvasEmotionWorkspace node={emotionNode} viewport={viewport} containerRef={containerRef} onClose={() => setEmotionNodeId(null)} onConfirm={(payload) => generateEmotionNode(emotionNode, payload)} /> : null}

            {maskEditNode?.metadata?.content ? (
                <CanvasNodeMaskEditDialog dataUrl={maskEditNode.metadata.content} open={Boolean(maskEditNode)} onClose={() => setMaskEditNodeId(null)} onConfirm={(payload) => void maskEditImageNode(maskEditNode!, payload).catch(() => undefined)} />
            ) : null}
            ...
            {splitNode?.metadata?.content ? (
                <CanvasNodeSplitDialog dataUrl={splitNode.metadata.content} open={Boolean(splitNode)} onClose={() => setSplitNodeId(null)} onConfirm={(params) => void splitImageNode(splitNode!, params).catch(() => undefined)} />
            ) : null}

            {upscaleNode?.metadata?.content ? (
                <CanvasNodeUpscaleDialog dataUrl={upscaleNode.metadata.content} open={Boolean(upscaleNode)} onClose={() => setUpscaleNodeId(null)} onConfirm={(params) => void upscaleImageNode(upscaleNode!, params).catch(() => undefined)} />
            ) : null}
    ...
    if (creativeWorkspaceEnabled) {
        return (
            <CanvasCreativeWorkspaceChrome
                ...
                surface={canvasContent}
                overlays={<CanvasSelectionSizeOverlay adapter={surfaceClientAdapter} />}
                generationComposer={
                    workspaceComposerNode ? (
                        <CanvasNodePromptPanel
                            node={workspaceComposerNode}
                            variant="workspace"
                            isRunning={runningNodeId === workspaceComposerNode.id}
                            mentionReferences={mentionReferencesByNodeId.get(workspaceComposerNode.id) || []}
                            onPromptChange={handleNodePromptChange}
                            onConfigChange={handleConfigNodeChange}
                            onGenerate={handleGenerateNode}
                            onStop={confirmStopGeneration}
                            onAddReferenceFiles={createReferenceMediaNodes}
                            onImageSettingsOpenChange={(open) => {
                                setNodeImageSettingsOpen(open);
                                if (open) setToolbarNodeId(null);
                            }}
                ...
                onDeselect={deselectCanvas}
                selectedCount={selectedNodeIds.size}
                onDeleteSelected={() => deleteNodes(new Set(selectedNodeIds))}
                onClear={() => setClearConfirmOpen(true)}
                onGroup={groupSelectedNodes}
                canUngroup={canUngroup}
                onUngroup={ungroupSelectedNodes}
                ...
                backgroundMode={backgroundMode}
                showImageInfo={showImageInfo}
                onColorThemeChange={useThemeStore.getState().setTheme}
                onBackgroundModeChange={setBackgroundMode}
                onShowImageInfoChange={setShowImageInfo}
                accountControl={<UserStatusActions variant="canvas" />}
