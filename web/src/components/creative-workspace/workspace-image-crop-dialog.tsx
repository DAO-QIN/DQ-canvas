...
import { resolveWorkspaceImageDimensions, type WorkspaceImageCropRect, type WorkspaceImageDimensions } from "./workspace-image-data";

type DragMode = "move" | "resize";
type ResizeHandle = "n" | "e" | "s" | "w" | "ne" | "nw" | "se" | "sw";

const handles: ResizeHandle[] = ["nw", "n", "ne", "e", "se", "s", "sw", "w"];
const handleLabels: Record<ResizeHandle, string> = { nw: "左上", n: "上", ne: "右上", e: "右", se: "右下", s: "下", sw: "左下", w: "左" };
const minSize = 0.06;
const defaultCrop = { x: 0.12, y: 0.12, width: 0.76, height: 0.76 };

export type WorkspaceImageCropDialogProps = {
    dataUrl: string;
    open: boolean;
    initialCrop?: WorkspaceImageCropRect | null;
    sourceDimensions?: WorkspaceImageDimensions | null;
    onClose: () => void;
    onConfirm: (crop: WorkspaceImageCropRect) => void;
    afterClose?: () => void;
};

export function WorkspaceImageCropDialog({ dataUrl, open, initialCrop, sourceDimensions, onClose, onConfirm, afterClose }: WorkspaceImageCropDialogProps) {
    const boxRef = useRef<HTMLDivElement>(null);
    const [crop, setCrop] = useState<WorkspaceImageCropRect>(initialCrop || defaultCrop);
    ...
    useEffect(() => {
        if (open) setCrop(initialCrop || defaultCrop);
    ...
    const startDrag = (mode: DragMode, event: ReactPointerEvent, handle?: ResizeHandle) => {
        ...
        const move = (nextEvent: PointerEvent) => {
            const dx = (nextEvent.clientX - start.x) / box.width;
            const dy = (nextEvent.clientY - start.y) / box.height;
    ...
    return (
        <Modal title="裁剪图片" open={open && Boolean(dataUrl)} onCancel={onClose} afterClose={afterClose} footer={null} width={780} centered destroyOnHidden>
            <div className="space-y-4" data-testid="workspace-image-crop-dialog">
                ...
                <div className="flex items-center justify-end gap-2">
                    <Button onClick={() => setCrop(initialCrop || defaultCrop)}>重置</Button>
...
function CropMask({ crop }: { crop: WorkspaceImageCropRect }) {
    return (
        <>
...
export function moveCrop(crop: WorkspaceImageCropRect, dx: number, dy: number): WorkspaceImageCropRect {
    return { ...crop, x: clamp(crop.x + dx, 0, 1 - crop.width), y: clamp(crop.y + dy, 0, 1 - crop.height) };
}

export function resizeCrop(crop: WorkspaceImageCropRect, dx: number, dy: number, handle: ResizeHandle, locked: boolean, box: Pick<DOMRect, "width" | "height">): WorkspaceImageCropRect {
    const next = { ...crop };
...
function cropStyle(crop: WorkspaceImageCropRect) {
