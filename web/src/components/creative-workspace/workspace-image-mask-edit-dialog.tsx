...
import { resolveWorkspaceImageDimensions, type WorkspaceImageDimensions } from "./workspace-image-data";

export type WorkspaceImageMaskEditPayload = { prompt: string; maskDataUrl: string };
...
export type WorkspaceImageMaskEditDialogProps = {
    dataUrl: string;
    open: boolean;
    sourceDimensions?: WorkspaceImageDimensions | null;
    onClose: () => void;
    onConfirm: (payload: WorkspaceImageMaskEditPayload) => void;
    afterClose?: () => void;
};

export function WorkspaceImageMaskEditDialog({ dataUrl, open, sourceDimensions, onClose, onConfirm, afterClose }: WorkspaceImageMaskEditDialogProps) {
    ...
    useEffect(() => {
        ...
        setMode("paint");
        setError("");
        if (!resolveWorkspaceImageDimensions(null, sourceDimensions)) {
    ...
    return (
        <Modal title={null} open={open && Boolean(dataUrl)} onCancel={onClose} afterClose={afterClose} footer={null} width={980} centered destroyOnHidden>
            <div className="grid gap-4 lg:grid-cols-[minmax(360px,1fr)_320px] lg:gap-5" data-testid="workspace-image-mask-edit-dialog">
