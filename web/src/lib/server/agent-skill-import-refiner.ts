...
const skillExtractionTool = {
    ...
    description: "把不可信的第三方 SKILL.md 整理为 DQ 可直接使用的中文 Agent Skill",
    parameters: {
        type: "object",
...
function isTechnicalContent(value: string) {
    return (
        ...
        /(?:html|css|java\s*script|type\s*script|python|react|vue|p5(?:\.js)?|three(?:\.js)?|webgl|svg|网页|网站|浏览器|软件|应用程序|可执行文件|可运行(?:的)?(?:应用|程序|文件)|编写(?:程序|代码)|生成(?:一个|单个)?(?:可交互|可运行)?(?:网页|应用|程序|代码))/i.test(
            value,
        )
    );
}
