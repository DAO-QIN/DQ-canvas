export const ECOMMERCE_IMAGE_SKILL = {
    ...
    license: "MIT",
    enabled: true,
    workspaces: ["image", "canvas", "design"],
    action: "generate",
    ...
    instructions: `基于 buluslan/gpt-image2-ecommerce v0.1.0（MIT）适配到 DQ-绘图 后台模型执行。
...
执行时调用 DQ-绘图 后台已配置图片模型，不调用 Codex CLI、Shell、curl 或本地 HTTP 服务。`,
} as const;
