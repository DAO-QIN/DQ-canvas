...
export async function importAgentSkillFromGithub(input: { url: string; path?: string }): Promise<AgentSkillImportResult> {
    ...
    const repositoryName = `${location.owner}/${location.repository}`;
    const license = repository.license?.spdx_id || undefined;

    if (location.mode === "blob") {
        const sourcePath = requireSkillPath(location.path);
        return { repository: repositoryName, ref, candidates: [], skill: await readSkill(location, ref, sourcePath, license) };
    }
    ...
    if (requestedPath) {
        ...
        return { repository: repositoryName, ref, candidates: [], skill: await readSkill(location, ref, requestedPath, license) };
    }

    const entries = await githubJson<GitHubTreeResponse>(`/repos/${location.owner}/${location.repository}/git/trees/${encodeURIComponent(ref)}?recursive=1`, MAX_TREE_LENGTH);
    if (entries.truncated) {
        throw new GithubSkillImportError("GitHub 仓库目录过大，无法完整扫描 SKILL.md；请直接提供具体的 SKILL.md 或 tree 子目录地址", 413);
    }
    ...
    if (candidates.length > 1) return { repository: repositoryName, ref, candidates };
    return { repository: repositoryName, ref, candidates: [], skill: await readSkill(location, ref, candidates[0].path, license) };
}
...
async function githubText(url: string, maxLength: number): Promise<string> {
    ...
    try {
        response = await fetchSafeOutboundUrl(url, {
            headers: { Accept: "application/vnd.github+json", "User-Agent": "DQ-agent-skill-import" },
